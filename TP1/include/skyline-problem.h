...
class SkylineProblem {
  ...
  private:
    ...
    std::vector<Point> divide_and_conquer_algorithm(const std::vector<std::shared_ptr<Building>> buildings);
    std::vector<Point> divide_and_conquer_cutoff_algorithm(const std::vector<std::shared_ptr<Building>> buildings, uint32_t depth);
